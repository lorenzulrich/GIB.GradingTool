...
class TcPdf extends \TYPO3\TcPdf\Pdf {
	...
	public function Header() {
		$html = '';
		if ($this->PageNo() !== 1) { // Do this only on the first page
			$html .= '
<style>
	table.headerTable {
		color: #555555;
...
<table class="headerTable" width="100%">
	...
	public function Bookmark($txt, $level=0, $y=-1, $page='', $style='', $color=array(0,0,0), $x=-1, $link='', $chapterNumber='', $class='') {
		...
		$this->outlines[] = array('t' => $txt, 'l' => $level, 'x' => $x, 'y' => $y, 'p' => $page, 's' => strtoupper($style), 'c' => $color, 'u' => $link, 'cn' => $chapterNumber, 'cssClass' => $class);
	}
